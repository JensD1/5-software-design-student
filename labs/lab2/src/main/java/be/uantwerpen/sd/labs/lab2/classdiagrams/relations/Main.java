...
public class Main {
    public static void main(String[] args) {
        ...
        try { s1.book(new Attendee("Overflow"), "A4", price("12.00")); }
        catch (Exception ex) { System.out.println("[s1] " + ex.getMessage()); }

        s2.book(anna, "B1", price("10.00"));

        s3.book(bob, "C1", price("11.00"));
        s3.book(anna, "C2", price("11.00"));
        try { s3.book(new Attendee("Overflow-2"), "C3", price("11.00")); }
        catch (Exception ex) { System.out.println("[s3] " + ex.getMessage()); }
    ...
    private static BigDecimal price(String s) { return new BigDecimal(s); }
