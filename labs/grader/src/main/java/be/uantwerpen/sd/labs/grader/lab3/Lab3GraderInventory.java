...
public final class Lab3GraderInventory implements LabGrader {
    ...
    public Result run(SuiteRunner runner) {
        ...
        passed += runner.runSuite("Structure (types, inheritance, fields, access)", s1); total += s1.size();
        ...
        passed += runner.runSuite("API (contracts + @Override annotations)", s2); total += s2.size();
        ...
        passed += runner.runSuite("Behavior (events, listeners, wiring, concurrency)", s3); total += s3.size();
    ...
    private static boolean isPublicClass(Class<?> c) { return c != null && !c.isInterface() && Modifier.isPublic(c.getModifiers()); }
    private static boolean isAbstractClass(Class<?> c) { return c != null && Modifier.isAbstract(c.getModifiers()); }
    ...
    private static boolean isPublic(Method m) { return m != null && Modifier.isPublic(m.getModifiers()); }
    private static boolean isStatic(Method m) { return m != null && Modifier.isStatic(m.getModifiers()); }
    private static boolean returns(Class<?> ret, Method m) { return m != null && m.getReturnType() == ret; }
    ...
    private static Constructor<?> privateNoArgCtor(Class<?> c) {
        try { Constructor<?> cons = c.getDeclaredConstructor(); cons.setAccessible(true); return Modifier.isPrivate(cons.getModifiers()) ? cons : null; }
        catch (Throwable t) { return null; }
    }
    ...
    private static Object callStaticNoArg(Class<?> type, String method) {
        try { Method m = type.getDeclaredMethod(method); m.setAccessible(true); return m.invoke(null); } catch (Throwable t) { return null; }
    }

    private static Object call(Object target, String name, Class<?>[] sig, Object[] args) {
        try { Method m = target.getClass().getMethod(name, sig); m.setAccessible(true); return m.invoke(target, args); }
    ...
    private TestResult test_behavior_two_listeners_both_receive() {
        ...
        try {
            ...
            PCLProbe p1 = new PCLProbe(); PCLProbe p2 = new PCLProbe();
            add.invoke(db, p1); add.invoke(db, p2);
    ...
    private TestResult test_behavior_remove_is_idempotent() {
        ...
        try {
            ...
            rem.invoke(db, p);
            boolean ok;
            try { rem.invoke(db, p); ok = true; } catch (Throwable t) { ok = false; }
    ...
    private TestResult test_integration_controller_and_main_wiring() {
        ...
        try {
            ...
            String out = captureStdout(() -> { try { run.invoke(main); } catch (Throwable ignored) {} });
