...
public class Lab2GraderInterfaces implements LabGrader {
    ...
    private static final class Ctx {
        ...
        Ctx(Class<?> vd, Class<?> tv, Class<?> r, Class<?> cd, Class<?> ur) {
            VolumeDevice = vd; TV = tv; Radio = r; CDPlayer = cd; UniversalRemote = ur;
        }
    }
    ...
    public Result run(SuiteRunner runner) {
        ...
        passed += runner.runSuite("Structure (types, interface implementation, modifiers)", s1); total += s1.size();
        ...
        passed += runner.runSuite("API (method signatures & @Override presence)", s2); total += s2.size();
        ...
        passed += runner.runSuite("Behavior (state changes & polymorphism via remote)", s3); total += s3.size();
