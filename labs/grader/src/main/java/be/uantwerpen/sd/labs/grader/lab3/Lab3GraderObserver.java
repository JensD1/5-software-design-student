...
public final class Lab3GraderObserver implements LabGrader {
    ...
    public Result run(SuiteRunner runner) {
        ...
        passed += runner.runSuite("Structure (types & contracts)", s1); total += s1.size();
        ...
        passed += runner.runSuite("API (method signatures)", s2); total += s2.size();
        ...
        passed += runner.runSuite("Behavior (notification flow & state changes)", s3); total += s3.size();
    ...
    private static Class<?> tryLoad(String fqn) {
        try { return Class.forName(fqn); } catch (Throwable ignore) { return null; }
    }
    ...
    private static Method getDeclaredMethod(Class<?> c, String name, Class<?>... sig) {
        ...
        try { Method m = c.getDeclaredMethod(name, sig); m.setAccessible(true); return m; }
    ...
    private static Method findOneArgMethod(Class<?> type, String name) {
        ...
        for (Method m : type.getDeclaredMethods()) {
            if (m.getName().equals(name) && m.getParameterCount() == 1) { m.setAccessible(true); return m; }
        }
        return null;
    }
    ...
    private static Method findTwoArgMethod(Class<?> type, String name) {
        ...
        // also scan declared (non-public) to give better error messages
        for (Method m : type.getDeclaredMethods()) {
            if (m.getName().equals(name) && m.getParameterCount() == 2) { m.setAccessible(true); return m; }
        }
        return null;
    }
        ...
        @Override public Object invoke(Object proxy, Method method, Object[] args) {
            ...
            switch (name) {
                case "update" -> {
                    ...
                    } else { events.add(null); payloads.add(null); }
                    return null;
                }
    ...
    private Method findOneArgPublicMethodByName(Class<?> type, String name, Object arg) {
        ...
        for (Method m : type.getDeclaredMethods()) {
            ...
            if (m.getParameterCount() != 1) continue;
            Class<?> p = m.getParameterTypes()[0];
            if (arg == null || p.isInstance(arg)) { m.setAccessible(true); return m; }
        }
        return null;
    }
    ...
    private boolean place(K k, Object auction, Object bid, StringBuilder err) {
        try { getDeclaredMethod(k.Auction, "place", k.Bid).invoke(auction, bid); return true; }
        catch (InvocationTargetException ite) { if (err!=null) err.append("Cause: ").append(ite.getCause()); return false; }
        catch (Throwable t) { if (err!=null) err.append(t); return false; }
    }

    private Object highest(K k, Object auction) {
        try { return getDeclaredMethod(k.Auction, "highest").invoke(auction); }
        catch (Throwable t) { return null; }
    }

    private static Field getDeclaredFieldOrNull(Class<?> c, String name) {
        try { Field f = c.getDeclaredField(name); f.setAccessible(true); return f; } catch (Throwable t) { return null; }
    }

    private static boolean isPrivate(Field f) { return f != null && Modifier.isPrivate(f.getModifiers()); }
    ...
    private TestResult test_api_consoleAnnouncer_signatures() {
        ...
        try {
            ...
            boolean ctorOk;
            try { Constructor<?> c0 = Console.getDeclaredConstructor(); ctorOk = Modifier.isPublic(c0.getModifiers()); }
            catch (Throwable t) { ctorOk = false; }
    ...
    private TestResult test_api_maxBidTracker_signatures_and_override() {
        ...
        try {
            ...
            try { getMax     = Tracker.getMethod("getMax"); } catch (Throwable ignore) {}
            try { getHighest = Tracker.getMethod("getHighest"); } catch (Throwable ignore) {}
            try { highest    = Tracker.getMethod("highest"); } catch (Throwable ignore) {}
            boolean getterOk =
    ...
    private TestResult test_behavior_removeObserver_stops_notifications() {
        ...
        K k = k(); Object auction = newAuction(k);
        if (auction == null) return new TestResult(title, false, "Could not instantiate Auction.");
        Probe p1 = new Probe(); Probe p2 = new Probe();
        Object o1 = newObserverProxy(k, p1), o2 = newObserverProxy(k, p2);
        addObserver(k, auction, o1); addObserver(k, auction, o2);
    ...
    private TestResult test_behavior_duplicate_registration_not_duplicated() {
        ...
        K k = k(); Object auction = newAuction(k);
        if (auction == null) return new TestResult(title, false, "Could not instantiate Auction.");
        Probe p = new Probe(); Object o = newObserverProxy(k, p);
        addObserver(k, auction, o); addObserver(k, auction, o); // add twice
    ...
    private TestResult test_behavior_place_updates_highest_and_notifies_every_time_with_stable_event() {
        ...
        K k = k(); Object auction = newAuction(k);
        if (auction == null) return new TestResult(title, false, "Could not instantiate Auction.");
        Probe p = new Probe(); Object o = newObserverProxy(k, p); addObserver(k, auction, o);
        ...
        place(k, auction, b10, null); Object h1 = highest(k, auction);
        place(k, auction, b15, null); Object h2 = highest(k, auction);
        place(k, auction, b07, null); Object h3 = highest(k, auction);
        place(k, auction, b20, null); Object h4 = highest(k, auction);
    ...
    private TestResult test_behavior_detach_idempotent() {
        ...
        boolean ok;
        try { ok = removeObserver(k, auction, o); } catch (Throwable t) { ok = false; }
        return new TestResult(title, ok,
