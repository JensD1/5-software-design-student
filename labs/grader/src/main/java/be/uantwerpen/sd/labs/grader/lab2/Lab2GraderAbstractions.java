...
public class Lab2GraderAbstractions implements LabGrader {
    ...
    private static final class Ctx {
        ...
        Ctx(Class<?> s, Class<?> c, Class<?> q, Class<?> t) { Shape = s; Circle = c; Square = q; EquilateralTriangle = t; }
    }
    ...
    public Result run(SuiteRunner runner) {
        ...
        passed += runner.runSuite("Behavior (formulas, validation, polymorphism)", s3); total += s3.size();
    ...
    private boolean almost(double a, double b) { return !Double.isNaN(a) && Math.abs(a - b) < 1e-9; }
