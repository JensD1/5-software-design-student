...
public final class SuiteRunner {
    ...
    public SuiteRunner withColor(boolean enabled)   { this.color = enabled; return this; }
    public SuiteRunner withCompact(boolean enabled) { this.compact = enabled; return this; }
    public SuiteRunner withSilencedStudentOutput(boolean enabled) { this.silenceStudentStdout = enabled; return this; }

    // --- Tiny helpers (no extra deps) ---
    private String c(String code, String s) { return color ? code + s + "\u001B[0m" : s; }
    private String green (String s){ return c("\u001B[32m", s); }
    private String red   (String s){ return c("\u001B[31m", s); }
    private String yellow(String s){ return c("\u001B[33m", s); }
    private String dim   (String s){ return c("\u001B[2m",  s); }
    private String bold  (String s){ return c("\u001B[1m",  s); }
    private static String rpt(char ch, int n){ return (n<=0) ? "" : String.valueOf(ch).repeat(n); }
    ...
    public int runSuite(String title, java.util.List<java.util.function.Supplier<TestResult>> tests) {
        ...
        for (java.util.function.Supplier<TestResult> s : tests) {
            ...
            try {
                ...
                if (r.pass) passed++; else failed.add(String.format("%d) %s", idx, r.name));
            } finally {
    ...
    public String shortHint(String s) { return s; }
