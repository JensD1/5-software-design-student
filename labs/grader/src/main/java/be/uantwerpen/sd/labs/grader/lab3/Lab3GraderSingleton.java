...
public final class Lab3GraderSingleton implements LabGrader {
    ...
    public Result run(SuiteRunner runner) {
        ...
        passed += this.runner.runSuite("Structure (classes, constructors, modifiers)", s1); total += s1.size();
        ...
        passed += this.runner.runSuite("API (method signatures)", s2); total += s2.size();
        ...
        passed += this.runner.runSuite("Behavior (singleton identity & ID sequence)", s3); total += s3.size();
    ...
    private Class<?> cls(String simpleName) {
        for (String root : ROOTS) {
            ...
            try { return Class.forName(fqn); } catch (Throwable ignore) {}
        }
        return null;
    }

    private Ctx ctx() { return new Ctx(cls(SIMPLE)); }
    ...
    private static boolean notStatic(Method m) { return m != null && !Modifier.isStatic(m.getModifiers()); }

    private static Object callStaticNoArg(Method m) {
        try { m.setAccessible(true); return m.invoke(null); } catch (Throwable t) { return null; }
    }
