...
public class Lab2GraderInheritance implements LabGrader {
    ...
    private static final class Ctx {
        ...
        Ctx(Class<?> e, Class<?> p, Class<?> cs, Class<?> d) { Employee = e; Programmer = p; CustomerService = cs; DepartmentOfficer = d; }
    }
    ...
    public Result run(SuiteRunner runner) {
        ...
        passed += runner.runSuite("Structure (classes, inheritance, modifiers)", s1); total += s1.size();
        ...
        passed += runner.runSuite("API (constructors & methods)", s2); total += s2.size();
        ...
        passed += runner.runSuite("Behavior (salary calculations & polymorphism)", s3); total += s3.size();
    ...
    private boolean declaresMethodHere(Class<?> c, String name) {
        if (c == null) return false;
        try { c.getDeclaredMethod(name); return true; } catch (Throwable t) { return false; }
    }

    private static String miss(String what) { return "Missing or incorrect " + what + "."; }
    ...
    private TestResult test_behavior_employeeSalary() {
        ...
        Ctx k = ctx(); SafeRef R = new SafeRef();
